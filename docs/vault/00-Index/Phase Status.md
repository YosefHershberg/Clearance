...
> [!info] Current
> **Phase 4b — Codegen + execute + self-correct** · status **not-started** · branch *(to create)* `feat/buildcheck-phase-4b`
>
...
| 4b | Codegen + execute + self-correct (v3.1 visual bridge) | **not-started** | — | — | Sidecar `/render-thumbnails` + `/execute`, explorer emits raw+decoded samples, multimodal Opus codegen with thumbnails, `ExtractionScript` cache (global, append-only), state-machine handler, `DxfFile.complianceData` populated |
